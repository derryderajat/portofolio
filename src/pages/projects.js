...
const FeaturedProject = ({ type, title, summary, img, link, github }) => {
  return (
    ...
    shadow-2xl p-12 rounded-br-2xl"
    >
      ...
      <div className="w-1/2  flex flex-col items-start justify-between pl-6">
        <span className="text-primary dark:text-dark font-medium text-xl">
          {type}
        </span>
        <Link
          href={link}
...
const Project = ({ type, title, img, link, github }) => {
  return (
    <article className="w-full flex flex-col items-center justify-center rounded-2xl border boreder-solid border-dark bg-light dark:border-light dark:bg-dark p-6 relative max-h-[400px]">
      <div className="absolute top-0 -right-3 -z-10 w-[101%] h-[103%] rounded-[2rem] bg-dark dark:bg-light rounded-br-3xl" />
      <Link
        href={link}
        target="_blank"
